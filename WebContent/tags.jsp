...
<body>
	<ol>
		...
		<li>property 取值为字符串: <s:property value="'username'" />
		</li>
		<li>property 设定默认值: <s:property value="admin" default="管理员" />
		</li>
		...
		<li>set 使用#取值: <s:property value="#adminPassword" />
		</li>
		<li>set 从相应范围取值: <s:property value="#session.adminPassword" />
		</li>
		...
		<li>bean 定义bean,并使用param来设定新的属性值: <s:bean
				name="com.struts2.sags.Dog">
				<s:param name="name" value="'pp'"></s:param>
				<s:property value="name" />
			</s:bean>
		</li>

		<li>bean 查看debug情况: <s:bean name="com.struts2.sags.Dog"
				var="myDog">
				<s:param name="name" value="'oudy'"></s:param>
			</s:bean> 拿出值： <s:property value="#myDog.name" />
		</li>
		<hr />
		不建议使用 ，只是说%的用意；%代表使用ognl
		<li>include _include1.html 包含静态英文文件 <s:include
				value="/_include1.html"></s:include>
		</li>

		<li>include _include2.html 包含静态中文文件 <s:include
		...
		<li>if elseif else: age = <s:property value="#parameters.age[0]" /><br /> 
			<s:set var="age" value="#parameters.age[0]" /> <br /> 
			<s:if test="#age < 0">wrong age!</s:if> <br /> 
			<s:elseif test="#parameters.age[0] < 20">too young!</s:elseif> <br /> 
			<s:else>yeah!</s:else><br />
			<s:if test="#parameters.aaa == null">null</s:if><br /> 
		</li>
		<hr />
